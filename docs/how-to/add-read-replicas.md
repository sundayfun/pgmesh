...
Replica selection is visible on every `pgmesh.query.duration` data point. Group
that metric by `pgmesh.shard.name` and `pgmesh.node.name` to get each replica's
QPS and latency. Nodes are named by configuration order (`replica-0`,
`replica-1`, and so on); explicit-primary and write routes use `primary`.
